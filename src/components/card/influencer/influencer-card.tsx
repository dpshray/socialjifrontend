...
export default function InfluencerInsightsCard({
    ...
    const totalFollowers = social_profiles.reduce((acc, p) => acc + Number(p.follower_count || 0), 0)
        ...
        <Card
            ...
            <CardContent className="p-4 flex flex-col flex-1">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 text-center">
                    <div>
                        <div className="text-sm font-bold text-gray-900">{formatCompactNumber(totalFollowers)}</div>
                        <div className="text-xs text-gray-600">Followers</div>
                    </div>
                    <div>
                        <div
                            className="text-sm font-bold text-green-500">${formatCompactNumber(parseFloat(highest_price_gig) || 0)}</div>
                        <div className="text-xs text-gray-600">Highest Gig Price</div>
                    </div>
                    <div>
                        <div
                            className="text-sm font-bold text-blue-500">${formatCompactNumber(parseFloat(lowest_price_gig) || 0)}</div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4 text-center">
                    <div>
                        <div
                            className="text-sm font-bold text-gray-900">${formatCompactNumber(parseFloat(highest_price_gig) || 0)}</div>
